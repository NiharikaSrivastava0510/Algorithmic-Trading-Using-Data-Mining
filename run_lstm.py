#!/usr/bin/env python3
"""
run_lstm.py — LSTM Neural Network Training Pipeline.
=====================================================
...
# Prerequisites:
#     run_feature_engineering.py must have been executed first so that
#     outputs/artefacts/train_prepared.csv exists.
...
#     # LOGISTIC CALIBRATION — P(spread > 0)
#     # ═══════════════════════════════════════════════════════════
#     # NOTE: Calibrator is CREATED HERE first, then saved below
#     _header("Logistic Calibration: spread to P(spread > 0)")
...
#     # SAVE MODEL & ARTEFACTS
#     # ═══════════════════════════════════════════════════════════
#     # NOTE: calibrator is saved HERE — after it was created above
#     _header("Saving Model and Artefacts")

#     joblib.dump(
#         calibrator,
#         os.path.join(cfg.ARTEFACT_DIR, "spread_calibrator.pkl")
#     )
#     print("  spread_calibrator.pkl  saved")
...
#     ])
#     history_df.to_csv(
#         os.path.join(cfg.ARTEFACT_DIR, "training_history.csv"), index=False
#     )
...
#     submission.to_csv(
#         os.path.join(cfg.ARTEFACT_DIR, "submission.csv"), index=False
#     )
#     print(f"  submission.csv         saved  ({len(submission):,} probability forecasts)")
#     print(f"  P(spread>0) range:     [{all_probs.min():.4f}, {all_probs.max():.4f}]")
#     print(f"  P(spread>0) mean:      {all_probs.mean():.4f}")
#     print(f"  Raw EUR range:         [{all_preds_eur.min():.2f}, {all_preds_eur.max():.2f}]")
